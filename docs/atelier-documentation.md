...
### 1. Telemetría IoT, Diagnóstico Asistido por Inteligencia Artificial y Reportes de Salud Vehicular
El servicio premium y diferencial que los talleres ofrecen a sus clientes mediante hardware OBD2 (módems SIM celulares o adaptadores BLE/WiFi). Atelier monitoriza constantemente los PIDs (datos sensoriales en tiempo real como velocidad, RPM, temperatura del motor y tensión eléctrica) y los códigos de falla (DTC bajo normas SAE J2012 e ISO 15031-6). Ante cualquier anomalía, el sistema envía una alerta simultánea al taller y a la aplicación móvil del cliente (Atelier Driver), permitiendo actuar antes de que ocurra una avería grave.
Más allá de la detección inmediata, Atelier incorpora un **Motor de Diagnóstico Predictivo Asistido por Inteligencia Artificial** sustentado en Spring AI y la plataforma de inferencia ultra-rápida de **Groq Cloud LPU** (con el modelo fundacional `llama-3.3-70b-versatile`). Este motor analiza las series de tiempo almacenadas en TimescaleDB y el historial de averías para inferir fallas mecánicas complejas (desgaste de bujías por *misfire*, degradación de alternador o riesgo de sobrecalentamiento térmico), clasificando la severidad clínica del automotor mediante un semáforo de salud (Bueno, Precaución, Peligro), persistiendo automáticamente alertas de alta certeza (>= 70%) y generando informes periciales completos exportables en formato PDF para el conductor y el jefe de taller.
...
### 3. Directorio de Proveedores, Órdenes de Compra y Gestión de Lotes
Módulo administrativo diseñado para el reabastecimiento eficiente y la gobernanza documental del taller. El sistema ofrece dos modalidades complementarias adaptadas a la operativa diaria:
* **Abastecimiento Formal Multi-Producto (`purchase_orders` y `purchase_order_items`):** Permite emitir órdenes de compra a proveedores homologados (`suppliers`) consolidando múltiples repuestos, fluidos y consumibles distintos en una sola transacción formal. Al llegar la mercadería, el usuario sube el escaneo de la factura o boleta física (PDF o imagen) **una sola vez** a nivel de la orden mediante *Direct-to-Cloud* en Firebase Storage y registra su número correlativo fiscal. Al confirmar la recepción física, el sistema genera de manera automática e individual los lotes FIFO correspondientes (`inventory_batches`) para cada producto adquirido, calculando sus saldos y costos unitarios sin requerir subidas duplicadas de archivos.
* **Entrada Rápida de Mostrador (`inventory_batches`):** Para compras menores de emergencia en locales cercanos donde no se requiere emitir una orden de compra previa, el administrador puede crear un lote de abastecimiento directo sobre el repuesto, registrando costo, cantidad y adjuntando la fotografía del comprobante de caja.

### 4. MRO (Órdenes de Trabajo) y Trazabilidad
El núcleo operativo del taller, estructurado desde el momento en que se genera una Cita o arribo directo vehicular.
* **Flujo Operativo y Roles:** Una Cita confirmada o arribo en recepción se convierte en una Orden de Trabajo (OT) gestionada por el Asesor de Servicio. El asesor divide la reparación en múltiples Tareas de Mano de Obra (`work_order_tasks`), asigna mecánicos según especialidad y asocia los repuestos presupuestados.
* **Hallazgos Periciales y Propuestas de Tareas (`task_proposals`):** Durante la intervención en elevador o foso, los mecánicos no pueden crear tareas aprobadas ni alterar arbitrariamente la cotización. Si detectan averías imprevistas o vicios ocultos, registran un *hallazgo pericial* con evidencia fotográfica (subida directamente a Firebase Storage) y emiten una propuesta de tarea. El Asesor de Servicio recibe la propuesta, prepara el presupuesto y actúa como puente pedagógico y de confianza humana con el conductor para explicar la causa técnica y solicitar su autorización. Si el cliente la aprueba, la propuesta se convierte en una tarea formal en estado pendiente; si la rechaza, permanece documentada en el historial clínico vehicular para soporte de mantenimiento predictivo futuro.
* **Registro Visual y Financiero:** La OT centraliza las imágenes del vehículo al ingresar (`work_order_images`) y la evidencia pericial de la reparación finalizada (`work_order_task_images`). Además, las tareas y repuestos mantienen su propio costo integrado (`price`, `unit_price`), permitiendo que el cliente visualice en tiempo real la liquidación transparente de su servicio.
* **Ejecución y Desacoplamiento REST (*Shallow Routing*):** Los mecánicos interactúan únicamente con sus tareas asignadas a través de endpoints desacoplados de segundo nivel (`/api/v1/tasks/{taskId}/...`), evitando sobrecargas de anidamiento profundo. Al agregar repuestos o lubricantes a una labor técnica, el sistema emite una reserva lógica inmediata contra el módulo de Inventario, transformándose en deducción física y costeo FIFO definitivo al conciliar el pago formal de la orden.
...
* **Groq Cloud LPU & Spring AI (Diagnóstico Predictivo Asistido por Inteligencia Artificial y Reportes de Salud):**
  * **Propósito y Casos de Uso:** Proveer un motor de inferencia de lenguaje de altísima velocidad y bajo costo para transformar millones de lecturas telemáticas numéricas e historiales de averías DTC en diagnósticos mecánicos causales y explicables. Permite que el asesor de servicio y el conductor reciban un análisis clínico automotriz profundo en menos de 1 segundo.
  * **Implementación con Spring AI y Neutralidad de Proveedor (*Vendor Neutrality*):** La arquitectura backend utiliza `spring-ai-openai-spring-boot-starter` acoplado mediante la interfaz desacoplada `ChatClient` hacia el endpoint compatible de Groq (`https://api.groq.com/openai/v1`). La adaptación perimetral (`VehicleHealthAiDiagnosticService`) compila un vector estadístico mediante agregaciones continuas `time_bucket()` de TimescaleDB y averías activas de `vehicle_faults`, invocando al modelo fundacional `llama-3.3-70b-versatile` con un `BeanOutputConverter` para estructurar la respuesta en un Java 21 Record fuertemente tipado (`VehicleHealthReportAiDto`). Este diseño garantiza estricta neutralidad de proveedor: mediante simples variables de entorno en `application.yml`, el taller puede conmutar entre Groq Cloud LPU (producción interactiva a ~500 tokens/s), NVIDIA NIM (`meta/llama-3.1-70b-instruct`) o un despliegue soberano on-premise con Ollama (`llama3.1:8b`) sin modificar código Java.
  * **Generación Pericial de Reportes en PDF (OpenPDF y Thymeleaf):** A través del adaptador `VehicleHealthReportPdfGeneratorAdapter`, el resultado estructurado se procesa en una plantilla XHTML tipográfica estilizada con CSS e inyectada mediante Thymeleaf, compilando un documento binario PDF descargable (`GET /api/v1/iot/health-reports/{reportId}/pdf`) que incluye el membrete del taller, odómetro certificado, estado clínico general (semáforo de salud), desglose por subsistemas vehiculares y paquetes de servicio preventivo recomendados.
  * **Viabilidad Económica:** Utiliza el plan permanente de desarrollador de Groq Cloud LPU (14,400 solicitudes gratuitas al día), eliminando costos de infraestructura de inferencia y asegurando sostenibilidad financiera para el modelo de negocio SaaS.
...
3. **Workshop Operations Context (MRO - Maintenance, Repair, and Operations):**
   * **Propósito y Por qué se creó:** Es el motor central del taller mecánico. Se delimitó para orquestar exclusivamente el flujo de trabajo físico y la consistencia transaccional de las reparaciones, manteniendo los precios y subtotales locales de cada orden (para facilidad de cómputo en el frontend) sin verse abrumado por detalles de facturación tributaria nacional (Invoicing) o suscripciones (SaaS Billing).
   * **Descripción:** Controla el ciclo de vida de las Órdenes de Trabajo (`work_orders`) gestionadas por el Asesor de Servicio, desglosándolas en tareas atómicas (`work_order_tasks`) operadas en foso por los mecánicos. Incorpora el flujo de hallazgos periciales y propuestas (`task_proposals`), donde averías ocultas descubiertas por los técnicos son validadas por el asesor con el conductor mediante diálogo humano antes de formalizarse. Centraliza el almacenamiento pericial *Direct-to-Cloud* en Firebase Storage, expone una API desacoplada con *Shallow Routing* (`/api/v1/tasks/{id}/products`) y coordina reservas lógicas con el módulo de Inventario que se consolidan en deducción FIFO al liquidar el pago.
...
4. **Inventory and Supply Chain Context:**
   ...
   * **Descripción:** Funciona como el core logístico. Garantiza la trazabilidad estricta de repuestos y líquidos mediante el método de costeo FIFO por Lotes físicos (`inventory_batches`). Gestiona el ciclo completo de abastecimiento mediante Órdenes de Compra formales (`purchase_orders` y `purchase_order_items`), permitiendo adquirir múltiples repuestos en una misma compra y centralizar el comprobante fiscal escaneado (`receipt_image_url`) una sola vez a nivel de la orden, para luego instanciar automáticamente los lotes individuales de cada ítem al recibir la mercadería. Asimismo, administra el directorio de proveedores homologados con validación de RUC ante SUNAT (`suppliers`) y soporta ingresos rápidos de mostrador para compras directas de emergencia.
...
5. **Human Resources Management Context:**
   ...
   * **Descripción:** Gestiona los turnos laborales, el cálculo de planillas/salarios de los mecánicos y el control de asistencia validada algorítmicamente mediante GPS y geocercas satelitales. Automatiza la deducción por tardanzas o faltas, emite boletas de pago en PDF al correo del trabajador vía Resend y genera la trama de exportación oficial en formato de texto plano (`.rem`) para la carga masiva en el aplicativo PLAME de SUNAT.

6. **Invoicing and Compliance Context:**
   * **Propósito y Por qué se creó:** La tributación (impuestos, XMLs fiscales) y la salud financiera del taller demandan aislamiento de la mecánica operativa. Se creó para encapsular las reglas impuestas por la SUNAT y proveer gobernanza del flujo monetario del negocio.
   * **Descripción:** Responsable de la emisión de comprobantes de pago electrónicos con validez legal (UBL 2.1) operando detrás de una Capa Anticorrupción (ACL) hacia Nubefact. Asimismo, centraliza el motor de Reportes Financieros y Flujo de Caja (*Cash Flow & Movements Statement*), unificando cronológicamente los cobros de clientes en MRO (`voucher_payments`), los pagos de compras a proveedores (`purchase_orders`) y los desembolsos salariales (`payroll_payments`) en un estado de cuenta bancario descargable en PDF con saldo acumulado progresivo.
...
8. **IoT Telemetry and Predictive Maintenance Context:**
   * **Propósito y Por qué se creó:** Manejar el altísimo volumen de datos estructurados por tiempo (Time-Series) que generan los escáneres OBD2, evitando colapsar y ralentizar las bases de datos transaccionales (PostgreSQL) del ERP del taller, e independizar el pipeline de analítica predictiva avanzada asistida por Inteligencia Artificial.
   * **Descripción:** El núcleo de la innovación de Andeva. Se encarga de la ingesta masiva de telemetría (vía tarjetas SIM celulares o vía la aplicación móvil actuando como Gateway Bluetooth Low Energy) particionándola en hipertablas temporales en *TimescaleDB* con políticas automáticas de compresión columnar. Alberga motores analíticos deterministas en memoria (`PredictiveAnomalyDetectionEngine`) para detectar sobrecalentamiento y fallas eléctricas en milisegundos, despachando alertas push reactivas mediante Firebase FCM. Asimismo, integra el pipeline pericial de salud vehicular asistido por IA sustentado en Spring AI y Groq Cloud LPU (`llama-3.3-70b-versatile`), persistiendo alertas predictivas de alta probabilidad (>= 70%) en `predictive_alerts`, recomendando paquetes de servicio en MRO y renderizando informes de auditoría técnica vehicular en formato binario PDF mediante OpenPDF y Thymeleaf.
